...
def build_parser() -> argparse.ArgumentParser:
    ...
    analyze.add_argument("--processor", help="explicit processor/model selected by the investigator")
    analyze.add_argument("--byte-order", choices=("big", "little"))
    analyze.add_argument("--ghidra-export", help="validated JSON export produced by a Ghidra script")
...
def run_analyze(args: argparse.Namespace) -> int:
    profile = profile_binary(
        args.firmware, processor=args.processor, byte_order=args.byte_order
    )
